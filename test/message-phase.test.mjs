...
test("ambiguous event data disables phase rewriting and preserves bytes", async () => {
  for (const bad of [
    'event: response.completed\ndata: {"type":"response.failed","type":"response.completed"}\n\n',
    'event: response.failed\ndata: {"type":"response.completed"}\n\n',
    'data: {"type":"response.completed","n":9007199254740993}\n\n',
    'data: {broken}\n\n',
  ]) {
    const input = messageFrames(0, message("m1", "Pending")).join("") + bad +
      block({ type: "response.completed", response: { output: [message("m1", "Pending")] } });
    assert.equal((await label(input, { chunkSize: 11 })).toString(), input);
  }
});

test("an oversized fragmented frame releases pending data unchanged", async () => {
  const input = messageFrames(0, message("m1", "Pending")).join("") +
    'data: ' + 'x'.repeat(10 * 1024 * 1024 + 1) + '\n\n' +
    block({ type: "response.completed", response: { output: [] } });
  assert.equal((await label(input, { chunkSize: 65536 })).toString(), input);
});

test("failure prevents a later success from assigning a phase", async () => {
  const input = block({ type: "response.failed" }) +
