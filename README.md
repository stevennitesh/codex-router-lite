...
GLM-5.3-Flash is available under its named model ID on OpenCode Go,
OpenRouter, Z.ai API, Z.ai Coding, and Ollama Cloud. The model accepts text and
image input where the provider route exposes them, uses a low/high/max reasoning
ladder, and defaults to max.

| Picker label | Model ID | Needs a key | Status |
| --- | --- | --- | --- |
| GLM-5.3-Flash (opencode Go) | `opencode-go/glm-5.3-flash` | opencode | Available |
