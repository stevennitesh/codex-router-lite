...
import { MODEL_BY_GATEWAY_ID, MODEL_BY_SLUG, PROVIDERS, resolveProviderBaseUrl } from "./routed-models.mjs";
...
function sanitizePayload(payload) {
  const selected = MODEL_BY_GATEWAY_ID.get(payload?.model) ||
    ...
    (payload?.model === route.upstreamModel ? route : undefined);
  if (selected !== route) throw new Error("Only openrouter/glm-5.3-flash is supported.");
  if (hasSearch(payload)) {
    const error = new Error("OpenRouter GLM-5.3-Flash does not support Codex hosted search.");
  ...
  return {
    ...
    provider: route.openRouterProviderPolicy,
  };
}
